...
namespace MVCApp.Models
{
    ...
    public class PhotoService
    {
        ...
        public static List<Photos> GetPhotosByPage(int pageInt)
        {
            long pageIndex = (long)pageInt;
            string sql = "select * from Photos";
            if (pageIndex > 0)
            {
                sql = string.Format("select * from Photos limit {0},{1}", (pageIndex - 1) * Config.PageSize, Config.PageSize);
            }
        ...
        {
            string sql = "update Photos set Title=@Title,Path=@Path where Id=@Id";
            SQLiteParameter[] param = 
            {
                new SQLiteParameter("@Title",DbType.String,50),
                new SQLiteParameter("@Path",DbType.String,50),
            ...
            param[0].Value = photo.Title;
            param[1].Value = photo.Path;
            param[2].Value = photo.Id;
            using (SQLiteConnection conn = new SQLiteConnection(ConnectionString))
            {
                conn.Open();
                using (SQLiteCommand cmd = new SQLiteCommand(sql, conn))
